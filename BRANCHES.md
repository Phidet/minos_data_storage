...
What each branch in the `NtpSt` tree means, and — for those the archive
leaves out — why. The manifest itself is [`branches.txt`](branches.txt);
this is the reasoning behind it. See [README.md](README.md) for running the
tool, and [`SCHEMA.md`](SCHEMA.md) for the derivations behind the
redundancy claims below.
